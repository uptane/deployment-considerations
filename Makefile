...
html: ## Create an HTML version of the deployment considerations, using docker
	@docker run --rm -it -v $(PWD):/data uptane/pandoc $(MKD) --toc --filter pandoc-include --metadata=title:"Uptane Deployment Best Practices v.$(RELEASE_VERSION)" -o $(HTML) --self-contained

pdf: ## Create a PDF version of the deployment considerations, using docker
	@docker run --rm -it -v $(PWD):/data uptane/pandoc $(MKD) --toc --filter pandoc-include --metadata=title:"Uptane Deployment Best Practices v.$(RELEASE_VERSION)" -o $(PDF)

...
Similarly, on the Image repository, the Timestamp and Snapshot metadata SHOULD be signed using online keys so fresh metadata can be instantly generated by an automated process. There are two options for signing the Timestamp and Snapshot metadata, each with the opposite trade-off from the other. In the first option, the OEM uses online keys, meaning automated processes for renewing the Timestamp and Snapshot metadata to indicate when new Targets metadata and/or images are available. However, in this scenario, attackers who have compromised a supplier's key as well as the Image repository could now instantly publish malicious images. If these attackers also compromise the Director repository, then they can execute arbitrary software attacks by selecting these malicious images on the Image repository for installation. Such an attack could also facilitate mix-and-match attacks.

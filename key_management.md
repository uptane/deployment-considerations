...
Second, the OEM SHOULD manually recall all vehicles in order to replace these keys. This MAY be done by requiring vehicle owners to visit the nearest dealership. Even though it could replace these keys on a full verification ECU by using over-the-air broadcasts, a manual recall SHOULD be done because:

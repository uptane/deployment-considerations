...
In the first option, which we expect to be the common case, a fleet manager would configure the map file on ECUs such that Primaries and full verification Secondaries would trust an image if, and only if, it has been signed by both the OEM-managed Image repository, and the fleet-managed Director repository. Partial verification Secondaries would trust an image, if and only, if it has been signed by this Director repository. The upside of this option is that the fleet manager, instead of the OEM, has complete control over which updates are installed on its vehicles. The downside of this option is that should the fleet-managed Directory repository be compromised, attackers can execute mix-and-match attacks.
